...
//Argument growing indicates whether the network is to be growing (growing != 0) or not (growing == 0).
unsigned long network_new(int growing);

//If there exists a network with given id then it is deleted, otherwise nothing is done.
void network_delete(unsigned long id);

//If there exists a network with given id then returns the number of nodes it contains, otherwise returns 0.
size_t network_nodes_number(unsigned long id);

//If there exists a network with given id then returns the number of links it contains, otherwise returns 0.
